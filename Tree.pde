...
void drawTrees(ArrayList<tree> trees) {
  for (int j = 0; j < trees.size(); j++) {
    int tempTreeSize = trees.get(j).treeSize;
    trees.get(j).swing();
    stroke(80, 0, 50, 200);
    for(int i = 1; i < count; i ++) {
      strokeWeight(trees.get(j).twig[(int)trees.get(j).map[i].x].thickness[(int)trees.get(j).map[i].y]);
      line(trees.get(j).twig[(int)trees.get(j).map[i].x].location[(int)trees.get(j).map[i].y - 1].x, trees.get(j).twig[(int)trees.get(j).map[i].x].location[(int)trees.get(j).map[i].y - 1].y,
           trees.get(j).twig[(int)trees.get(j).map[i].x].location[(int)trees.get(j).map[i].y].x, trees.get(j).twig[(int)trees.get(j).map[i].x].location[(int)trees.get(j).map[i].y].y);
    }

    noStroke();
    for(int i = 0; i < trees.get(j).twig.length; i++) {
      int num = trees.get(j).twig[i].location.length - 1;
      ellipse(trees.get(j).twig[i].location[num].x, trees.get(j).twig[i].location[num].y, 12, 12);
    }
  }
